...
public class WS_Solicita_Autorizacion {
    ...
    public int solicitaAutorizacion(@WebParam(name = "idClte") int idClte, @WebParam(name = "idProv") int idProv, @WebParam(name = "dblMonto") double dblMonto) throws Excepcion_Negocio {
        ...
        if(clte == null)
            //return autorizacion;
            throw new Excepcion_Negocio("CREDITO: El cliente con id " + idClte + " No EXISTE en las Autorizaciones de Crédito");
        
        prov = ejbProv.find(idProv);
        
        if(prov == null)
            //return autorizacion;
            throw new Excepcion_Negocio("CREDITO: El proveedor con id " + idProv + " No EXISTE en las Autorizaciones de Crédito");
        
        if(dblMonto <= 0.0)
            //return autorizacion;
            throw new Excepcion_Negocio("CREDITO: Se ha solicitado monto menor a 0.5");
        
        BigDecimal monto_solicitado = new BigDecimal(dblMonto);
        
        if( clte.getMontoMax().compareTo(monto_solicitado) == -1)
            //return autorizacion;
            throw new Excepcion_Negocio("CREDITO: El monto solicitado " + dblMonto + " es mayor que el máximo permitido");
        ...
        if( monto_disponible.multiply(new BigDecimal(1.05)).compareTo(monto_solicitado) == -1 )
            //return autorizacion;
            throw new Excepcion_Negocio("CREDITO: El monto solicitado " + dblMonto + " sobrepassa el monto disponible "  );
